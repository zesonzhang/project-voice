/** Validates per-platform, real-device compatibility evidence. */

import {existsSync, readFileSync} from 'node:fs';
import {resolve} from 'node:path';

const root = resolve('.');
const matrixPath = resolve(root, 'docs/on-device/compatibility.json');
const evidenceDir = resolve(
  process.argv[2] ||
    process.env.ON_DEVICE_COMPATIBILITY_RESULTS ||
    'docs/on-device/results',
);
...
console.log(`real-device compatibility evidence PASSED: ${evidenceDir}`);
