/**
 * Validates a real-device M4.7 soak result. This command intentionally does
...
const resultPath = process.argv[2] || process.env.M4_SOAK_RESULT;
if (!resultPath) {
  throw new Error(
    'BLOCKED: provide a real-device result path: npm run test:m4-soak -- <result.json>',
  );
}
...
for (const [name, passed] of gates) {
  if (!passed) throw new Error(`M4.7 release gate failed: ${name}`);
}

console.log(`M4.7 real-device soak evidence PASSED: ${absolutePath}`);
