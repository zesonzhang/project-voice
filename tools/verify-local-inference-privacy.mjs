...
console.log('Verifying static privacy boundary invariants...');
...
// 4. Test Suite presence: test_local-inference-privacy.ts
const testPrivacyPath = resolve(ROOT, 'src/tests/test_local-inference-privacy.ts');
if (!existsSync(testPrivacyPath)) {
  console.error('FAIL: src/tests/test_local-inference-privacy.ts not found');
  process.exit(1);
}
console.log('✓ Browser privacy regression suite is present.');

console.log(
  'Static boundary checks PASSED. This source inspection does not measure wire bytes; run the browser suite and an independent deployed-origin network audit.',
);
