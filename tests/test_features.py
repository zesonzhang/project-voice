...
class FeaturesApiTest(unittest.TestCase):
  ...
  def test_get_features_default_values(self):
    ...
    self.assertIn('internalUser', data)
    self.assertEqual(data['debugModelImport'], False)
    self.assertEqual(data['rolloutPercentage'], 100)

  def test_invalid_mode_fails_closed(self):
    with mock.patch.dict(os.environ, {'FEATURE_ON_DEVICE_MODE': 'unexpected'}):
      response = self.client.get('/api/features')
      self.assertEqual(response.get_json()['onDeviceMode'], 'disabled')

  def test_internal_mode_uses_server_session_membership(self):
    with mock.patch.dict(os.environ, {'FEATURE_ON_DEVICE_MODE': 'internal'}):
      response = self.client.get('/api/features')
      self.assertEqual(response.get_json()['internalUser'], False)

      with self.client.session_transaction() as session:
        session['is_internal_user'] = True
      response = self.client.get('/api/features')
      self.assertEqual(response.get_json()['onDeviceMode'], 'internal')
      self.assertEqual(response.get_json()['internalUser'], True)
