...
export interface FeatureFlags {
  ...
  /** Server-asserted cohort membership; never inferred from client input. */
...
export function resolveSafeInferenceMode(
  ...
  const allowed = isClientInOnDeviceCohort(flags, clientId);
    ...
    return {
      ...
      statusMessage: !allowed
        ? isInstalledLocally
          ...
          : 'On-device inference is paused. Local mode is preserved to prevent an unexpected cloud request, but a model cannot be installed.'
        : undefined,
    };
  }
    ...
    this.fetchPromise = (async () => {
      try {
        ...
        if (response.ok) {
          ...
          this.flags = {
            onDeviceMode:
              data.onDeviceMode === 'internal' ||
              data.onDeviceMode === 'canary' ||
              data.onDeviceMode === 'all'
                ? data.onDeviceMode
                : 'disabled',
