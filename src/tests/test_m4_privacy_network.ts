...
const M4_TEST_MANIFEST: ModelManifest = {
  schemaVersion: 1,
...
describe('M4.5 End-to-End Privacy & Network Verification', () => {
  let state: State;
  ...
  beforeEach(async () => {
    ...
    state = new State(new ConfigStorage('test-m4-privacy', CONFIG_DEFAULT));
    ...
    await metadataStore.saveModel({
      modelId: M4_TEST_MANIFEST.modelId,
      activeVersion: M4_TEST_MANIFEST.version,
      lastKnownGoodVersion: M4_TEST_MANIFEST.version,
      updatedAt: Date.now(),
    });
    await metadataStore.saveVersion({
      modelId: M4_TEST_MANIFEST.modelId,
      version: M4_TEST_MANIFEST.version,
      manifest: M4_TEST_MANIFEST,
      fileName: `${M4_TEST_MANIFEST.version}.litertlm`,
      partialFileName: `${M4_TEST_MANIFEST.version}.partial`,
      sizeBytes: M4_TEST_MANIFEST.sizeBytes,
      sha256: M4_TEST_MANIFEST.sha256,
      gcsGeneration: M4_TEST_MANIFEST.gcsGeneration,
      downloadOffset: M4_TEST_MANIFEST.sizeBytes,
    ...
    spyOn(storage, 'getModelFileSize').and.returnValue(
      Promise.resolve(M4_TEST_MANIFEST.sizeBytes),
    );
    spyOn(storage, 'openModelFile').and.returnValue(
      Promise.resolve(
        new File(
          [new Uint8Array(M4_TEST_MANIFEST.sizeBytes)],
          'model.litertlm',
        ),
      ),
    );
    ...
    spyOn(window, 'fetch').and.callFake(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('/api/on-device-models/default')) {
          return new Response(JSON.stringify(M4_TEST_MANIFEST), {
            status: 200,
    ...
    localProvider = new LocalSuggestionProvider(
      ...
      () => ({
        modelId: M4_TEST_MANIFEST.modelId,
        modelVersion: M4_TEST_MANIFEST.version,
      }),
  ...
  it('proves zero network calls and zero data leaks during continuous typing and suggestion updates in Local mode', async () => {
  ...
  it('proves zero network calls and zero fallback when in-flight generation is rapidly cancelled', async () => {
  ...
  it('proves zero network calls and zero silent cloud fallback when runtime errors occur', async () => {
  ...
  it('proves zero network calls and zero fallback when local model is unloaded', async () => {
  ...
  it('proves zero model-byte network downloads on page restart/reload in Local mode', async () => {
    capturedRequests = [];
  ...
  it('proves update check transmits only metadata and never leaks user text or prompts', async () => {
    capturedRequests = [];
  ...
  it('proves removing the installed model never causes silent fallback to Cloud', async () => {
    capturedRequests = [];
    ...
    await modelManager.removeModel(
      M4_TEST_MANIFEST.modelId,
      M4_TEST_MANIFEST.version,
    );
