...
describe('M4.11 Privacy-Safe Local Diagnostics Export', () => {
  ...
  it('integrates export button cleanly into PvSettingPanel', async () => {
    const state = new State(
      new ConfigStorage('test-m4-diag-panel', CONFIG_DEFAULT),
    );
    state.inferenceMode = 'local';
    const panel = new PvSettingPanel();
    panel.state = state;
    panel.modelManager = manager;
    document.body.appendChild(panel);
    await panel.updateComplete;

    const content = (panel.renderRoot as HTMLElement).innerHTML;
    expect(content).toContain('Export Diagnostics');
    expect(content).toContain('Export privacy-safe diagnostics report');
