...
describe('On-device integration and privacy', () => {
  let state: State;
  ...
  it('verifies complete CUJ: mode switch, auto-load, local inference, and zero network calls', async () => {
    ...
    const confirmHealthySpy = spyOn(
      modelManager,
      'confirmActiveVersionHealthy',
    ).and.callThrough();
    ...
    expect(confirmHealthySpy).toHaveBeenCalled();
    expect(
      modelManager
        .getTransitionHistory()
        .some(transition => transition.to === 'generating'),
    ).toBeTrue();
    expect(modelManager.getState()).toBe('ready');
