...
describe('Feature-Flag and Rollout Controls', () => {
  it('defaults to Cloud mode, disables debug import, and enables general cohort', () => {
    expect(DEFAULT_FEATURE_FLAGS.onDeviceMode).toBe('disabled');
    expect(DEFAULT_FEATURE_FLAGS.debugModelImport).toBeFalse();
    expect(DEFAULT_FEATURE_FLAGS.rolloutPercentage).toBe(0);
    expect(DEFAULT_FEATURE_FLAGS.internalUser).toBeFalse();
  });
  ...
  it('correctly filters cohorts across disabled, canary, and percentage rollouts', () => {
    ...
    expect(isClientInOnDeviceCohort(disabledFlags, 'any-client')).toBeFalse();
  ...
  it('allows internal rollout only from server-asserted membership', () => {
    ...
    expect(isClientInOnDeviceCohort(flags, 'client')).toBeFalse();
    expect(
      isClientInOnDeviceCohort({...flags, internalUser: true}, 'client'),
    ).toBeTrue();
  });
  ...
  it('fetches server feature flags and parses cohort configurations', async () => {
    ...
    expect(flags.internalUser).toBeTrue();
  });
});
