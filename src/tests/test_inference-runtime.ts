...
import {RuntimeStatusEvent} from '../on-device/model-runtime-adapter.js';
  ...
  loseDevice() {
    this.respond({
      ...
      error: {
        ...
        message: 'WebGPU device lost: test',
        phase: 'error',
        recoverable: true,
      },
    });
...
describe('Inference runtime and Worker protocol', () => {
  ...
  describe('InferenceWorkerClient', () => {
    ...
    it('rejects active work and publishes a recoverable error on device loss', async () => {
      const worker = new ProtocolFakeWorker();
      const client = new InferenceWorkerClient({
        workerFactory: () => worker as unknown as Worker,
      });
      const file = new File([new Uint8Array(16)], 'model.litertlm');
      await client.load({...TEST_MANIFEST, sizeBytes: file.size}, file);
      const events: RuntimeStatusEvent[] = [];
      client.onStatusChange(event => events.push(event));

      const read = client
        .generate('device loss request', {sequenceId: 7})
        [Symbol.asyncIterator]()
        .next();
      worker.loseDevice();

      await expectAsync(read).toBeRejectedWithError(/device lost/i);
      expect(client.getStatus()).toBe('error');
      const lastEvent = events[events.length - 1];
      expect(lastEvent?.errorCode).toBe('WEBGPU_DEVICE_LOST');
      expect(lastEvent?.recoverable).toBeTrue();
    });
  });
