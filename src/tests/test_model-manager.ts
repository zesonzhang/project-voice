...
describe('ModelManager Lifecycle and Failure Injection', () => {
  ...
  it('enforces manifest storage requirements and reports low-memory warnings', async () => {
    const constrainedManifest = {
      ...testManifest,
      requirements: {
        ...testManifest.requirements,
        minimumFreeStorageBytes: 1_000_000,
      },
    };
    const manager = createTestManager({
      quotaEstimator: async () => ({quota: 500_000, usage: 0}),
      deviceMemoryChecker: () => 4,
    });

    const preflight = await manager.checkCapabilities(
      constrainedManifest.sizeBytes,
      constrainedManifest.adapterId,
      constrainedManifest.requirements,
    );

    expect(preflight.supported).toBeFalse();
    expect(preflight.errorCode).toBe('ERR_INSUFFICIENT_STORAGE');
    expect(preflight.memoryWarning).toContain('below the recommended minimum');
  });

  it('rejects a runtime that is not cross-origin isolated', async () => {
    const manager = createTestManager({
      crossOriginIsolatedChecker: () => false,
    });

    const preflight = await manager.checkCapabilities();

    expect(preflight.supported).toBeFalse();
    expect(preflight.errorMessage).toContain('Cross-origin isolation');
  });
  ...
  it('recovers from interrupted activation / failed smoke test without corrupting existing active model', async () => {
    ...
    await manager.initialize();
    await expectAsync(manager.updateModel(v2Manifest)).toBeRejectedWithError(
      /smoke test failed/i,
    );

    // The update error is surfaced to the caller while v1 is restored and
    // remains available for inference.
    expect(manager.getState()).toBe('ready');
    expect(manager.getActiveManifest()?.version).toBe('v1');
    expect(await storage.hasModel(testManifest.modelId, 'v1')).toBeTrue();
