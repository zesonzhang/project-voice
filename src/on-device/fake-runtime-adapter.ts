...
export class FakeModelRuntimeAdapter implements ModelRuntimeAdapter {
  ...
  private emitStatus(event: RuntimeStatusEvent): void {
  ...
  async load(manifest: ModelManifest, file: File): Promise<void> {
    ...
    if (this.options.loadShouldFail) {
      this.emitStatus({status: 'error', errorMessage: 'Load failed'});
  ...
  async *generate(
    ...
    this.generatedCount++;
    this.emitStatus({status: 'generating'});
  ...
  async cancel(): Promise<void> {
    this.emitStatus({status: 'canceling'});
    this.isCancelled = true;
    this.emitStatus({status: this.isLoaded ? 'ready' : 'idle'});
  }
