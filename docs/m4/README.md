...
| **M4.4** | Cloud Download Auth | Session + CSRF, rate limiting, generation pinning | **Blocked on live GCS/IAM evidence** |
| **M4.5** | Privacy Verification | `tools/verify-m4-privacy-network.mjs`, `src/tests/test_m4_privacy_network.ts` | **Code test complete; independent audit pending** |
| **M4.6** | Compatibility Matrix | `docs/m4/compatibility-matrix.md`, `docs/m4/compatibility.json`, `tools/verify-m4-compatibility.mjs` | **Blocked on real-device evidence** |
| **M4.7** | Performance & Soak | `docs/m4/performance-soak.md`, `tools/m4-soak-runner.mjs` | **Blocked on real-device evidence** |
| **M4.8** | Failure Injection | `docs/m4/failure-injection.md`, `src/tests/test_m4_failure_injection.ts` | **Complete** |
| **M4.9** | Accessibility (A11y) | `docs/m4/accessibility.md`, component tests | **Partial; manual review pending** |
...
| **M4.13** | Release Audit & Sign-off | `docs/m4/audit.md` | **Blocked** |
...
4. **Performance SLOs:** These are release targets, not current results: warm first-word p95 <= 2.0s; complete result p95 <= 5.0s; 0 inference tasks > 200ms; <10% memory growth over 30 minutes; parse rate >=95%.
5. **Accessibility target:** Automated semantics are implemented. WCAG 2.1 AA must not be claimed until the manual review in `accessibility.md` is complete.
