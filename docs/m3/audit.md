...
| **M3.6** | Separate inference source and Cloud model in Settings | Settings UI | Complete | In `src/pv-setting-panel.ts`, separated Inference Source (`Cloud (Gemini)` vs `On-device`) from Cloud AI Model selection. Tested in `src/tests/test_on_device_settings.ts`. |
| **M3.7** | Build On-Device Model Card and lifecycle actions | Settings UI | Complete | `src/pv-on-device-model-card.ts` renders lifecycle, metadata and actions; the parent Settings surface owns destructive confirmation. Tested in `src/tests/test_on_device_model_card.ts` and `src/tests/test_on_device_settings.ts`. |
| **M3.8** | Implement user-facing Local errors and recovery | UX / privacy | Complete | Localized actionable errors and explicit recovery actions preserve the zero-silent-cloud invariant. Tested in model-card, integration and privacy suites. |
| **M3.9** | Build resource-status telemetry panel | Settings UI / telemetry | Complete | The model card exposes CPU, RAM, OPFS, compatibility, page/Worker memory and runtime metrics. Tested in `src/tests/test_on_device_model_card.ts`. |
...
| **M3.11** | Enforce accessibility (A11y) across Settings | Accessibility | Code complete | Live regions, progress semantics and a sibling alertdialog are covered by component tests; manual M4 accessibility qualification remains pending. |
| **M3.12** | Comprehensive test suite & verification record | Testing & QA | Complete | Runtime, integration, app, model-card and Settings suites are included in the browser test bundle. Current counts are recorded in the M4 audit. |
...
npm run test:js
# See `docs/m4/audit.md` for the current recorded count.
