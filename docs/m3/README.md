...
- `npm run test:js`: runs the current browser regression suite; see `docs/m4/audit.md` for the latest recorded count.
...
1. **UI Layer Decomposition (`src/pv-setting-panel.ts`):**
   ...
   - Model card encapsulated in `src/pv-on-device-model-card.ts`; Settings owns the sibling confirmation dialog.
...
4. **Current Verification Baseline:**
   - Current verification status and known global-lint findings are recorded in `docs/m4/audit.md`.
