...
| Apply rollout and kill-switch policy | `src/feature-flags.ts` |
...
| Card/dialog templates | `src/on-device/model-card-template.ts` |
...
- UI change: run accessibility coverage in `src/tests/test_m4_accessibility.ts`.
